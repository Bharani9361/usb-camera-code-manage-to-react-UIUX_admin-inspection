...
const admin_accuracy_single = () => {
  ...
  const cameras = React.useMemo(() => {
    return (selectedStage?.camera_selection?.cameras || []).filter(
      camera => camera.training_status === 'training completed'
    );
  ...
  const fetchGalleryImages = async () => {
    ...
    try {
      const response = await urlSocket.post('/Admin_accuracy_image', {
      ...
      try {
        ...
        for (let cam of cameras) {
          ...
          try {
            ...
            console.log(
              `Found matching device for ${cam.label}:`,
              videoDevice.label
            );
  ...
  const handleCaptureAll = async () => {
    ...
    try {
      ...
      await urlSocket.post('/admin_remoteMultiCapture', formData, {
  ...
  const handleDeleteImage = async (filename, cameraLabel) => {
    ...
    try {
      await urlSocket.post('/admin_accuracy_delete_training_image', {
        filename,
            ...
            >
              ...
              >
                {capturing ? (
                  ...
                  <>
                    <i className='fa fa-camera me-2' /> Capture All
                  </>
                )}
              </Button>
            ...
            <Row className='justify-content-center'>
              <Col xs={12} md={8} lg={6}>
                <Card
                  ...
                  style={{ borderRadius: '10px', backgroundColor: '#fff3cd' }}
                >
                  <CardBody className='text-center py-5'>
                    <i
                      className='mdi mdi-alert-circle-outline'
                      style={{ fontSize: '48px', color: '#856404' }}
                    />
                    <h5
                      className='mt-3'
                ...
                <div style={{ flex: 1, paddingLeft: '12px' }}>
                  ...
                  <Row className='g-3 mb-4'>
                    {cameras.map((camera, index) => {
                      ...
                      const outlineImgUrl = outlinePath
                        ? showImage(outlinePath)
                        : null;
...
export default admin_accuracy_single;
