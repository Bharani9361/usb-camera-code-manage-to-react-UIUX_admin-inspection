...
const InspectionTestingStageSequential = () => {
    ...
    const [currentStageIndexforButton, setCurrentStageIndexforButton] = useState(0);
    const [sequential_insp_id, setsequential_insp_id] = useState('');
    const [seq_region_resuls, setseq_region_resuls] = useState([]);
    // const [batch_id, setBatchId] = useState('');
    console.log('seq_region_resuls', seq_region_resuls)


    useEffect(() => {
        const map = {};
        seq_region_resuls.forEach(group => {
            const item = group[0];
            map[item.camera_label] = item.status;
        });
        setResponse_message(map);
    }, [seq_region_resuls]);
    ...
    // useEffect(async () => {  due to asyc in this code error occurs so changed to below
    ...
    useEffect(() => {
        ...
        async function init() {
            try {
                ...
                console.log('compData.batch_id', compData.batch_id)


                if (!isMounted || !compData) return;
                ...
                setCompData(compData);
                setShow(true);
                setInspection_type(compData.inspection_type);
                setQr_checking(compData.qr_checking);
                setQruniq_checking(compData.qruniq_checking);
                setCapture_duration(Number(compData.timer));
                setQrbar_check(compData.qrbar_check);
                setQrbar_check_type(compData.qrbar_check_type);
                setTimeout_val(Number(compData.timer) * 1000);

                // Component identity setup
                setComponent_name(component_name1);
                setComponent_code(component_code1);
                setComp_id(compData._id);
                ...
                checkWebcam();
                navigator.mediaDevices.addEventListener("devicechange", checkWebcam);
                barcodeInfo(compData._id, compData.inspection_type, compData);

            } catch (error) {
                console.error("Error inside useEffect init:", error);
            }
        }
    ...
    // Updated useEffect to draw rectangles only for the active camera/stage
    useEffect(() => {
        console.log('=== Rectangle Drawing Effect ===');
        ...
        if (!canvasRef.current || !activeCamera) {
            console.warn('Canvas ref or activeCamera not available');
            return;
        }

        // Get the canvas for the ACTIVE camera only
        const canvasEl = canvasRef.current[activeCamera.label];

        if (!canvasEl || typeof canvasEl.getContext !== 'function') {
            console.warn(`Canvas not found for active camera: ${activeCamera.label}`);
            return;
        }

        const ctx = canvasEl.getContext('2d');
        if (!ctx) {
            console.error(`Failed to get context for camera: ${activeCamera.label}`);
            return;
        }

        // Clear canvas first
        ctx.clearRect(0, 0, canvasEl.width, canvasEl.height);
        console.log(`Canvas cleared for ${activeCamera.label}`);

        // Get rectangles for the current active camera
        const currentRectangles = rectanglesByCamera[activeCamera.label];

        // Draw rectangles if they exist for this camera
        if (currentRectangles && currentRectangles.length > 0) {
            console.log(`✅ Drawing ${currentRectangles.length} rectangles for ${activeCamera.label}`);

            currentRectangles.forEach((rect, index) => {
                console.log(`Drawing rectangle ${index + 1}:`, rect);
            ...
            console.log(`✅ Successfully drew all rectangles for ${activeCamera.label}`);
        } else {
            console.log(`ℹ️ No rectangles to draw for ${activeCamera.label}`);
        }

        // Dependency: re-run when active camera changes, rectangles change, or stage changes
    }, [rectanglesByCamera, activeCamera, currentStageIndex]);
    ...
    const refreshDetailsCount = async () => {
        try {
            ...
            if (batch_data.length === 0) {
                ...
                setOkCount(Number(batch_data[0].ok));
                setNgCount(Number(batch_data[0].notok));
                setPsMatch(Number(batch_data[0].posbl_match));
                setTCount(Number(batch_data[0].total));
            }
        } catch (error) {
            console.error(error);
        }
    ...
    const object_detection = async () => {
        ...
        const camerasToCapture = [cameraList[currentStageIndex]];
        console.log('camerasToCapture', camerasToCapture)
        const totalStage = cameraList.length;
        const currentStage = currentStageIndex;
        console.log('firsttotalStage', currentStage, totalStage)
        ...
        formData.append('totalStage', totalStage);
        formData.append('sequential_insp_id', sequential_insp_id || null);
        ...
        try {
            ...
            setShowstatus(true);
            if (totalStage === (currentStage + 1)) {
                console.log('totalstage', totalStage)
                console.log('currentStage', currentStage + 1)
                setsequential_insp_id('')
            }
            else {
                setsequential_insp_id(response.data[0].insp_result_id)
            }
            ...
            setTimeout(async () => {
                if ((compData.detect_selection === true && hasObjectDetected) ||
                    ...
                    try {
                        ...
                        setseq_region_resuls(prev => [...prev, detection.data.prediction]);
                        ...
                        seq_region_resuls.forEach(group => {
                            const item = group[0];
                            resultMap[item.camera_label] = item.status;
                        });
    ...
    const convertStageDataToRectangles = (stageIndex) => {
        console.log('convertStageDataToRectangles called for stage:', stageIndex);
        console.log('seq_region_resuls:', seq_region_resuls);

        // Check if data exists for this stage
        if (!seq_region_resuls || seq_region_resuls.length === 0) {
            console.warn('No seq_region_resuls data available');
            return {};
        }

        if (stageIndex < 0 || stageIndex >= seq_region_resuls.length) {
            console.warn(`Stage index ${stageIndex} out of bounds`);
            return {};
        }

        // Get data for current stage
        const currentStageData = seq_region_resuls[stageIndex];
        console.log('currentStageData:', currentStageData);

        if (!currentStageData || currentStageData.length === 0) {
            console.warn(`No data for stage ${stageIndex}`);
            return {};
        }

        // Scale factors (same as in your object_detection function)
        const originalWidth = 640;
        const originalHeight = 480;
        const targetWidth = DEFAULT_RESOLUTION.width;
        const targetHeight = DEFAULT_RESOLUTION.height;
        ...
        // Process each camera result in this stage
        currentStageData.forEach((cameraResult) => {
            console.log('Processing cameraResult:', cameraResult);

            // Convert backend label (position1) to frontend label (Position1)
            const backendLabel = cameraResult.camera_label.charAt(0).toUpperCase() +
                cameraResult.camera_label.slice(1);
            console.log('backendLabel:', backendLabel);

            // Find matching camera in cameraList
            const matchingCamera = cameraList.find(cam =>
                cam.label === backendLabel &&
                cam.stageName === cameraResult.stage_name
            );
            ...
            // Map region_results to rectangle format
            const retrievedRectangles = cameraResult.region_results.map((rect) => ({
                x: rect.rectangles.coordinates.x / scaleX,
                y: rect.rectangles.coordinates.y / scaleY,
                height: rect.rectangles.coordinates.height / scaleY,
            ...
            console.log('retrievedRectangles for', frontendLabel, ':', retrievedRectangles);

            // Store rectangles for this camera
            rectanglesByCam[frontendLabel] = retrievedRectangles;
            console.log(`✅ Mapped ${backendLabel} → ${frontendLabel}`);
        });

        console.log('Final rectanglesByCam:', rectanglesByCam);
        return rectanglesByCam;
    };

    // Updated handleNextStageAndInspect
    const handleNextStageAndInspect = () => {
        if (currentStageIndex < cameraList.length - 1) {
            const nextStageIndex = currentStageIndex + 1;

            setCurrentStageIndex(nextStageIndex);
            setCurrentStageIndexforButton(currentStageIndex + 1);

            // Convert and set rectangles for next stage
            const rectanglesForNextStage = convertStageDataToRectangles(nextStageIndex);
            setRectanglesByCamera(rectanglesForNextStage);

            console.log('Moved to stage:', nextStageIndex);
    ...
    // Updated handlePreviousStage
    const handlePreviousStage = () => {
        if (currentStageIndex > 0) {
            const prevStageIndex = currentStageIndex - 1;

            setCurrentStageIndex(prevStageIndex);
            setCurrentStageIndexforButton(currentStageIndex);

            // Convert and set rectangles for previous stage
            const rectanglesForPrevStage = convertStageDataToRectangles(prevStageIndex);
            setRectanglesByCamera(rectanglesForPrevStage);

            console.log('Moved to stage:', prevStageIndex);
        }
    };

    // Updated handleNextComponent
    const handleNextComponent = () => {
        // Clear all data
        setRectanglesByCamera({});
        setShow(true)
        setseq_region_resuls([]);
        setCurrentStageIndex(0);
        setCurrentStageIndexforButton(0); // Display as "1" (first stage)
        setShow_next(false);
        setInspection_started(false);

        console.log('Reset to first stage for next component');
    };
    ...
    const test_opt = async () => {
        try {
            await urlSocket.post('/test_opt', { 'batch_id': batch_id }, { mode: 'no-cors' });
        } catch (error) {
            console.log("----", error);
        }
    };
                    ...
                    <Card>
                        <CardBody>
                            ...
                            <div style={{ padding: '20px' }}>
                                ...
                                {compData?.inspection_method === 'Sequential' && (
                                                    ...
                                                    <div style={{
                                                        ...
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                            <span style={{ fontWeight: '600', color: '#495057', fontSize: '16px' }}>
                                                                ...
                                                                Stage_Name : {activeCamera?.stageName}
                                                            </span>
                                                            <span style={{
                                                        ...
                                                        <div style={{ display: 'flex', gap: '8px' }}>
                                                            ...
                                                            {currentStageIndexforButton === cameraList.length ? (
                                                                ...
                                                                /* Next Component Button - Shows on last stage */


                                                                <Button
                                                                    color="primary"
                                                                    size="sm"
                                                                    onClick={handleNextStageAndInspect}
                                                                    style={{
                                                                        borderRadius: '6px',
                                                                        fontWeight: '600'
                                                                    }}
                                                                ...
                                                                {cameraDisconnected && cameraDisconnected[activeCamera?.originalLabel] ? (
                                                                    ...
                                                                    <>
                                                                        ...
                                                                        <div style={{
                                                                            ...
                                                                            {/* Result Icons */}
                                                                            {showresult && response_message && response_message[activeCamera?.label] && (
                                                                                <div style={{
                                                                                    display: 'flex',
                                                                                    flexDirection: 'column',
                                                                                    alignItems: 'center',
                                                                                    ...
                                                                                    {response_message[activeCamera?.label] === 'OK' && (
                                                                                        <>
                                                                                            ...
                                                                                            />
                                                                                            <div
                                                                                                style={{ fontSize: "24px", fontWeight: "bold", color: "#1e5a00ff" }}
                                                                                            >
                                                                                                Result:OK
                                                                                            </div>
                                                                                        </>
...
export default InspectionTestingStageSequential;
