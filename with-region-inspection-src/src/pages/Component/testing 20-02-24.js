...
class Inspection extends Component {
    ...
    componentDidMount = () => {
        ...
        console.log('comp_name, comp_code : ', component_name1, component_code1)
        try {
    ...
    cont_apiCall = () => {
        if(this.state.placeobj_count > 0){
            if(this.state.res_message === 'No Object Found'){                
                    ...
                    if (response.data[0].detection_result === 'Qr Code Found') {
                        ...
                        setTimeout(() => {
                            ...
                            if (testing_result === positive) {
                                ...
                                old_ok++
                                old_total++
                                this.setState({ response_message: positive, response_value: response.data.value, showresult: true,ok_count,old_ok,old_total, resultKey, result_key: true })
                            }
                            else if (testing_result === negative) {
                                // console.log('testing_result876', testing_result, negative)
                                let negative = testing_result.replaceAll(testing_result, this.state.config_negative)
                                ...
                                old_ng++
                                old_total++
                                this.setState({ response_message: negative, response_value: response.data.value, showresult: true,ng_count,old_ng,old_total, resultKey, result_key: true })
                            }
                            else if (testing_result === posble_match) {
                                let posble_match = testing_result.replaceAll(testing_result, this.state.config_posble_match)
                                console.log('posbl_match563', posble_match)
                                ...
                                old_pm++
                                old_total++
                                this.setState({ response_message: posble_match, response_value: response.data.value, showresult: true,ps_match,old_pm,old_total, resultKey, result_key: true })
                            }
                                this.cont_apiCall()
                        }, 400);
                    }
    ...
    object_detected = async (event) => {
        ...
        this.state.placeobj_count += 1  // to hide the Result at the starting
        console.log('this.state.placeobj_count (2) >>> <<< ', this.state.placeobj_count)
        clearInterval(this.state.intervalId)
        this.setState({ show: false, msg: false, show_next: false, showplaceobject: false, showresult: false, showstatus: false})
        const imageSrc = this.webcam.getScreenshot();
        const blob = await fetch(imageSrc).then((res) => res.blob());
        ...
        try {
            ...
            urlSocket.post('/time_sheet',{'id':batch_id},
                ...
                .then((response) => {
                    ...
                    let totalDurationInSeconds = 0, totalTotal = 0, totalOk = 0, totalNG = 0, totalNoObj = 0 ; 
                    ...
                    this.setState({ time_sheet: time_data, total_sn_tmsht: time_data.length, totalDuration1, totalOk, totalNG, totalNoObj, totalTotal, })
                })
                .catch((error) => {
                    console.log(error)
                })
    ...
    render() {
        ...
        const { showresult, response_message, resultKey, result_key, session_detail, report_data, manage_details, time_sheet, total_sn_tmsht, totalDuration1, station_name, totalOk, totalNG, totalNoObj, totalTotal, rtdt_TD, isCountdownActive} = this.state;
