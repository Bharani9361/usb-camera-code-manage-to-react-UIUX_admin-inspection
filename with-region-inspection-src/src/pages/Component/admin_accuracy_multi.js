...
import StageCameraGallery from './admin_accuracy_multi_gallery';
...
const admin_accuracy_multi = () => {
  ...
  const [completedcameras, setCompletedcameras] = useState([]);
  const [completedCamerasState, setCompletedCamerasState] = useState([]);
  const [selectedOutlineColor, setSelectedOutlineColor] =
    useState('White Outline');
  const [showOutline, setShowOutline] = useState(true);

  const isCameraTrainingCompleted = camera => {
    return camera?.training_status?.toLowerCase() === 'training completed';
  };
  ...
  useEffect(() => {
    ...
    const allCompleted = stages.flatMap(stage =>
      (stage.camera_selection?.cameras || [])
        .filter(camera => isCameraTrainingCompleted(camera))
        .map(camera => ({
          ...camera,
    ...
    setCompletedCamerasState(allCompleted);
  }, [stages]);

  console.log('All completed cameras:', completedCamerasState);

  useEffect(() => {
    const fetchExistingCounts = async () => {
      try {
        const response = await urlSocket.post('/Admin_accuracy_image_count', {
  ...
  const validateMissingPreviews = streamsByStage => {
    ...
    Object.entries(streamsByStage).forEach(([stageIndex, streams]) => {
      ...
      if (missingCameras.length > 0) {
        messages.push(
          `Camera preview missing in ${stage.stage_name
          } -> ${missingCameras.join(', ')}`
        );
      }
    });
  ...
  const sendCapturedImages = async capturedImages => {
    try {
      const response = await urlSocket.post('/admin_save_stage_camera_images', {
        capturedImages
      });
  ...
  const handleStageCameraSync = async () => {
    try {
      ...
      const res = await urlSocket.post(
        '/sync_training_mode_adminresult_train',
        {
          comp_name: datas.comp_name,
          comp_code: datas.comp_code
        }
        ...
        >
          ...
          >
            <div>
              ...
              >
                Component Name : {datas?.comp_name || 'Company'}
              </p>
            </div>
          </Col>
                ...
                <div key={stageIndex}>
                  <div style={{ marginBottom: '14px' }}>
                    ...
                    >
                      ...
                      >
                        stage name : {stage.stage_name} / stage code :{' '}
                        {stage.stage_code}
                      </h6>
                    </div>
                  </div>
                    ...
                    <div className='d-flex align-items-center gap-1'>
                      {outlineColors.map(color => (
                        <div
                          key={color}
                          onClick={() =>
                            showOutline && setSelectedOutlineColor(color)
                          }
                          style={{
                  ...
                  <div
                    style={{
                      display: 'flex',
                      alignItems: 'center',
                      gap: '12px', // reduce or increase spacing here
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              Image Count : {currentCount}/{stageLimit}
                            </span>
                          </div>
        ...
        {showGallery && (
          ...
          >
            <StageCameraGallery
              ...
              completedCamerasState={completedCamerasState}
            />
          </div>
        )}
...
export default admin_accuracy_multi;
