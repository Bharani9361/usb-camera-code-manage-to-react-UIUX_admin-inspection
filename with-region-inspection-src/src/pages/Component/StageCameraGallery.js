...
//       try {
//         const res = await urlSocket.post('/get_stage_camera_images', {
//           comp_code: compCode,
//           comp_name: compName
//         });
...
//     if (!selectedCamera) return;
//     const key = `${stageName}_${selectedCamera.camera_label}`;

//     try {
//       await urlSocket.post('/delete_multiple_stage_images', {
//         comp_code: compCode,
//         comp_name: compName,
//         image_paths: [imgPath]
//       });
...
//                   d => d.insp_local_path !== imgPath
//                 )
//               }
//             : cam
//         )
//       );
//       setSelectedImages(prev => prev.filter(p => p !== imgPath));
...
//     if (!selectedCamera || selectedImages.length === 0) return;
//     const key = `${stageName}_${selectedCamera.camera_label}`;
...
//                 ...cam,
//                 datasets: cam.datasets.filter(
//                   d => !selectedImages.includes(d.insp_local_path)
//                 )
//               }
//             : cam
//         )
...
//       setToastMessage(`${selectedImages.length} image(s) deleted successfully`);
//       setToastType('success');
//       setSelectedImages([]);
//       setCaptureProgress(prev => ({
//         ...prev,
...
const StageCameraGallery = ({
  ...
  const handleDeleteImage = async imgPath => {
    ...
    try {
      const res = await urlSocket.post('/delete_multiple_stage_images', {
        comp_code: compCode,
        comp_name: compName,
        image_paths: [imgPath]
      });
