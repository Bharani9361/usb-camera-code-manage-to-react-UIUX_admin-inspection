...
const Admin_accuracy_image = () => {
        ...
        return datas.stages
            .map(stage => {
                const completedCams = stage?.camera_selection?.cameras?.filter(
                    cam => cam.training_status === "training completed"
                ) || [];
    ...
    useEffect(() => {
        const loadAllCounts = async () => {
            ...
            for (let s = 0; s < stages.length; s++) {
                const stage = stages[s];
                try {
                    const { data } = await urlSocket.post("/Admin_accuracy_image", {
    ...
    const handleDeleteImage = async (image) => {
        ...
        try {
            const { data } = await urlSocket.post("/admin_accuracy_delete_training_image", {
    ...
    const handleBulkDelete = async () => {
        ...
        const promises = Array.from(selectedImages).map((fn) => {
            ...
            return urlSocket.post("/admin_accuracy_delete_training_image", {
                filename: fn,
    ...
    const handleManualCapturemode2 = async () => {
        ...
        try {
            ...
            showNotification(`Stage ${selectedStage + 1} images captured successfully!`);
                    ...
                    <CardBody className="py-3">
                        <Row className="align-items-center">
                            ...
                            >
                                ...
                                <Button size='sm' color="light" onClick={back}>
