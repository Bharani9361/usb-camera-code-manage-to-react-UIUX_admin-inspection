...
class ComponentAdd extends Component {
    ...
    submitForm = () => {
        ...
        try {
            axios.post('http://127.0.0.1:5000/add_comp', { 'comp_name': component_name, 'comp_code': component_code },
                { mode: 'no-cors' })
                .then((response) => {
                    let data = response.data
                    console.log(data)  
                                ...
                                <tbody>
                                    {
                                        this.state.componentList.map((data, index) => (
                                            <tr key={index}>
                                                ...
                                                {/* <td>{data._id}</td> */}
                                                <td>{data.status}</td>
                                                <td> <Link to="Camera"><Button onClick={() => this.editComp(data)}>Edit</Button></Link></td>
