...
const StageProfileRatioHandlerInsp = props => {
  ...
  const flattenGallery = data => {
    const all = [];

    Object.values(data).forEach(stageObj => {
      Object.values(stageObj).forEach(cameraArr => {
        all.push(...cameraArr); // spread the array into all
      });
    });
  ...
  const getTotalImageCount = data => {
    let count = 0;

    Object.values(data).forEach(stage => {
      Object.values(stage).forEach(cameraArray => {
  ...
  useEffect(() => {
    ...
    // Flatten the nested galleryData
    const flatImages = flattenGallery(galleryData);
    setGroupedGallery(flatImages);
    console.log('Flat Images:', flatImages);
    const total = getTotalImageCount(galleryData);
    setMaxSampleCount(total)
  ...
  //   if (!groupedGallery || groupedGallery.length === 0) {
  //     Swal.fire({
  //       icon: 'info',
  //       title: 'No loaded images',
  //       text: 'Please load images before starting test'
  ...
  //       const imagesForCamStage = groupedGallery.filter(
  //         g =>
  //           g.camera_label.trim().toLowerCase() ===
  ...
  //testing
  //done till overall result checking issue in mapping the result
  // const objectDetectionOnly = async () => {
  //   console.log('📸 Starting OBJECT DETECTION with LOADED IMAGES');
  ...
  //   const currentTestIndex = obj_count || 0;

  //   try {
  //     // ✅ Step 1: Collect ONE image per camera/stage for this test iteration
  //     const imagesToTest = [];
  ...
  //       const imageForThisTest = imagesForCamStage[currentTestIndex];

  //       if (!imageForThisTest) {
  //         console.warn(
  //           `No more images for ${cam.label} - ${cam.stageName} at index ${currentTestIndex}`
  //         );
  //         return;
  //       }
  ...
  //       Swal.fire({
  //         icon: 'warning',
  //         title: 'No more images to test',
  //         text: 'All available images have been tested'
  //       });
  //       return;
  //     }

  //     console.log(
  //       `🎯 Testing iteration ${currentTestIndex + 1} with ${
  //         imagesToTest.length
  //       } images`
  //     );
  ...
  //       formData.append('image_urls', fullUrl);
  //       formData.append('filenames', img.filename);
  //       formData.append('labels', img.camera.label);
  //       formData.append('versions', img.camera.model_ver);
  //       formData.append('stage_names', img.camera.stageName);
  //       formData.append('stage_codes', img.camera.stage_code);
  //       formData.append('stage_ids', img.camera.stage_id);
  //     });
  ...
  //     console.log('🟢 obj_detection Response:', response.data);

  //     // ✅ Add safety checks for response data
  //     if (!response.data) {
  //       throw new Error('No response data from detection API');
  //     }
  ...
  //     console.log('Captured Images:', capturedImagesData);

  //     // ✅ Step 4: Build detection status per camera
  //     const resultsByCamera = {};
  //     if (Array.isArray(detectionResults) && detectionResults.length > 0) {
  //       detectionResults.forEach(item => {
  //         const key = `${item.stage_name}_${item.camera_label}`;
  //         resultsByCamera[key] = item.detection_result;
  //       });
  //     }

  //     console.log('Detection Results by Camera:', resultsByCamera);
  //     setResMessage(resultsByCamera);
  //     setShowstatus(true);
  ...
  //     if (hasObjectDetected && !hasNoObject && !hasIncorrectObject) {
  //       // ✅ Step 6: Show "Checking..."
  //       setResMessage(prev => {
  //         const updated = { ...prev };
  //         detectionResults.forEach(item => {
  //           const key = `${item.stage_name}_${item.camera_label}`;
  //           updated[key] = 'Checking ...';
  //         });
  //         return updated;
  //       });
  ...
  //       console.log('🧩 overall_result Response:', detection.data);

  //       // ✅ Add safety check for detection.data
  //       if (
  //         !detection.data ||
  //         !Array.isArray(detection.data) ||
  //         detection.data.length === 0
  //       ) {
  //         throw new Error('Invalid response from quality inspection API');
  //       }

  //       setShowstatus(false);
  //       setCompFound(2);

  //       if (choosen_prof?.qrbar_check === true) setQrbarShowStart(true);
  //       else setShowStart(true);

  //       // ✅ Step 8: Process per-camera results with safety checks
  //       const cameraResults = {};
  //       const cameraValues = {};
  ...
  //         if (
  //           !cameraResult.region_results ||
  //           !Array.isArray(cameraResult.region_results)
  //         ) {
  //           console.warn('Missing region_results for camera:', cameraResult);
  //           return;
  //         }

  //         const key = `${cameraResult.stage_name}_${cameraResult.camera_label}`;
  //         const status = cameraResult.status;

  //         if (status === 'OK') {
  //           cameraResults[key] = config.positive;
  //         } else if (status === 'NG') {
  //           cameraResults[key] = config.negative;
  //         }

  //         // ✅ Safe max value calculation
  //         const values = cameraResult.region_results.map(r => r.value || 0);
  //         const maxValue = values.length > 0 ? Math.max(...values) : 0;
  //         cameraValues[key] = maxValue;
  //       });
  ...
  //       // ✅ Step 10: Map rectangles per camera with safety checks
  //       const originalWidth = 640;
  //       const originalHeight = 480;
  //       const targetWidth = DEFAULT_RESOLUTION?.width || 1920;
  //       const targetHeight = DEFAULT_RESOLUTION?.height || 1080;
  ...
  //         ) {
  //           console.warn(
  //             'Skipping rectangle mapping for camera:',
  //             cameraResult
  //           );
  //           return;
  //         }
  ...
  //         const retrievedRectangles = cameraResult.region_results.map(rect => {
  //           // ✅ Safety check for rectangle coordinates
  //           const coords = rect.rectangles?.coordinates || {};

  //           return {
  //             x: (coords.x || 0) / scaleX,
  //             y: (coords.y || 0) / scaleY,
  //             height: (coords.height || 0) / scaleY,
  //             width: (coords.width || 0) / scaleX,
  //             id: rect.rectangles?.id || '',
  //             name: rect.rectangles?.name || '',
  //             colour: rect.result === 'OK' ? 'green' : 'red',
  //             result: rect.result || 'UNKNOWN',
  //             value: rect.value || 0
  //           };
  //         });

  //         rectanglesByCam[key] = retrievedRectangles;
  //       });

  //       console.log('Rectangles by camera:', rectanglesByCam);
  //       setRectanglesByCamera(rectanglesByCam);
  //       setResMessage(cameraResults);
  //       setResponseValue(cameraValues);
  //       setOutputRect(true);
  ...
  //       const nextCount = (obj_count ?? 0) + 1;
  //       const reachedSampleTarget =
  //         Number.isFinite(sample_count) && nextCount >= sample_count;
  ...
  //       }
  //     } else {
  //       // ✅ No object detected or incorrect object
  //       setShowStart(choosen_prof?.qrbar_check === true ? false : true);
  //       setQrbarShowStart(choosen_prof?.qrbar_check === true ? true : false);
  //     }
  //   } catch (error) {
  //     console.error('❌ objectDetectionOnly Error:', error);
  ...
  //       icon: 'error',
  //       title: 'Testing Error',
  //       text: error.message || 'An error occurred during testing'
  //     });
  ...
  //     const detectionResults = Array.isArray(response.data?.detection_result)
  //       ? response.data.detection_result
  //       : [];
  //     const capturedImagesData = Array.isArray(response.data?.captured_image)
  //       ? response.data.captured_image
  //       : [];
  //     const updatedRectanglesList = Array.isArray(
  //       response.data?.updated_rectangles
  //     )
  //       ? response.data.updated_rectangles
  //       : [];
  ...
  //     // Step 4: Build detection status per camera
  //     const resultsByCamera = {};
  //     safeMap(detectionResults, item => {
  //       if (item.stage_name && item.camera_label) {
  //         const key = `${item.stage_name}_${item.camera_label}`;
  //         resultsByCamera[key] = item.detection_result || '';
  //       }
  //     });
  ...
  //       console.log('🧩 overall_result Response isworking:', detection.data);

  //       if (!Array.isArray(detection.data) || detection.data.length === 0)
  //         throw new Error('Invalid response from quality inspection API');

  //       setShowstatus(false);
  //       setCompFound(2);
  //       if (choosen_prof?.qrbar_check) setQrbarShowStart(true);
  //       else setShowStart(true);

  //       // Step 8: Process per-camera results
  //       const cameraResults = {};
  //       const cameraValues = {};
  //       const rectanglesByCam = {};

  //       // Group by stage_name + camera_label
  //       detection.data.forEach(item => {
  //         const key = `${item.stage_name}_${item.camera_label}`;
  //         if (!cameraResults[key])
  //           cameraResults[key] =
  //             item.status === 'OK' ? config.positive : config.negative;
  //         if (!cameraValues[key]) cameraValues[key] = item.status || '';
  //       });
  ...
  //       // Step 9: Determine overall status
  //       const allCameraStatuses = safeMap(
  //         detection.data,
  //         cam => cam.status || ''
  //       );
  //       const overallStatus = allCameraStatuses.every(status => status === 'OK')
  ...
  //       // Step 10: Handle rectangles
  //       if (Array.isArray(response.data?.region_results)) {
  //         const originalWidth = 640;
  //         const originalHeight = 480;
  //         const targetWidth = DEFAULT_RESOLUTION?.width || 1920;
  //         const targetHeight = DEFAULT_RESOLUTION?.height || 1080;
  ...
  //         response.data.region_results.forEach(regionData => {
  //           const key = `${regionData.stage_name}_${regionData.camera_label}`;
  //           const retrievedRectangles = safeMap(regionData.rectangles, rect => {
  ...
  //       // Step 12: Update OK/NG counters
  //       if (testing_result === 'OK') {
  //         const updated_ok = ok_count + 1;
  //         const updated_total = updated_ok + ng_count;
  //         setObjCount(nextCount);
  //         setShowRatio(show_ratio);
  //         setRatioData(ratio_data_);
  //         setCompResult(2);
  ...
  //         setTCount(updated_total);
  //       } else if (testing_result === 'NG') {
  //         const updated_ng = ng_count + 1;
  //         const updated_total = ok_count + updated_ng;
  //         setObjCount(nextCount);
  //         setShowRatio(show_ratio);
  //         setRatioData(ratio_data_);
  //         setCompResult(1);
  ...
  //     } else {
  //       // No object detected or incorrect object
  //       setShowStart(choosen_prof?.qrbar_check ? false : true);
  //       setQrbarShowStart(choosen_prof?.qrbar_check ? true : false);
  //     }
  //   } catch (error) {
  //     console.error('❌ objectDetectionOnly Error:', error);
  //     console.error('Error stack:', error.stack);
  ...
  const objectDetectionOnly = async () => {
    ...
    try {
      // ✅ Step 1: Collect ONE image per camera/stage for this iteration
      const imagesToTest = [];
      ...
      console.log(
        `🎯 Testing iteration ${currentTestIndex + 1} with ${imagesToTest.length
        } images`
      );

      // ✅ Step 2: Update image indices to show current test images
      const newImageIndices = {};
      imagesToTest.forEach(img => {
        const key = `${img.camera.stageName}_${img.camera.label}`;
        newImageIndices[key] = currentTestIndex;
      });
      setCameraImageIndex(newImageIndices);
      console.log('📸 Updated cameraImageIndex:', newImageIndices);
      ...
      // Step 4: Show status and send detection request
      setShow(false);
      setMsg(false);
      setShowNext(false);
      setShowplaceobject(false);
      setShowresult(false);
      setShowstatus(true); // ✅ Show "Object Detected" / "Checking..." status
      setSetValues(true);
      setShowStart(false);
      ...
      // ✅ Step 5: Show detection status (Object Detected, No Object, etc.)
      const resultsByCamera = {};
      detectionResults.forEach(item => {
        if (item.stage_name && item.camera_label) {
          const key = `${item.stage_name}_${item.camera_label}`;
          resultsByCamera[key] = item.detection_result || '';
          console.log(`✅ Detection: ${key} = ${item.detection_result}`);
        }
      });

      console.log('📊 Detection Results by Camera:', resultsByCamera);
      setResMessage(resultsByCamera); // ✅ Shows "Object Detected", "No Object", etc.
      setShowstatus(true); // ✅ Keep status visible
      ...
      // ✅ Delay to show detection status
      await new Promise(resolve => setTimeout(resolve, 1500));

      if (hasObjectDetected && !hasNoObject && !hasIncorrectObject) {
        // ✅ Step 7: Change to "Checking..." status
        console.log('✅ All objects detected, starting quality check...');
        ...
        setResMessage(prev => {
          ...
          detectionResults.forEach(item => {
            if (item.stage_name && item.camera_label) {
              const key = `${item.stage_name}_${item.camera_label}`;
              updated[key] = 'Checking ...';
              console.log(`🔄 Status: ${key} = Checking...`);
            }
          });
          return updated;
        });
        ...
        console.log('📤 Sending quality inspection request...');
        ...
        // ✅ Step 9: Hide "Checking..." and prepare to show final results
        setShowstatus(false); // ✅ Hide status overlay
        setCompFound(2);

        if (choosen_prof?.qrbar_check) setQrbarShowStart(true);
        else setShowStart(true);

        // ✅ Step 10: Process final results with rectangles
        const cameraResults = {}; // ✔ final OK/NG
        const cameraValues = {}; // ✔ numeric or NG/OK value
        const rectanglesByCam = {}; // ✔ rectangles
        ...
        detection.data.forEach(cameraResult => {
          ...
          // ✔ Final OK/NG - used in border colors
          cameraResults[key] = status;

          // ✔ Region values or fallback to OK/NG
          if (
            Array.isArray(cameraResult.region_results) &&
            cameraResult.region_results.length > 0
          ) {
            const values = cameraResult.region_results.map(r => r.value || 0);
            const maxValue = Math.max(...values);
            cameraValues[key] = maxValue;

            const retrievedRectangles = cameraResult.region_results.map(
              rect => {
                const coords = rect.rectangles?.coordinates || {};
        ...
        // ✔ Update UI for your existing components
        setRectanglesByCamera(rectanglesByCam);

        setResMessage(prev => ({
          ...prev,
          ...cameraResults // ✔ used in: "Result: OK/NG"
        }));

        console.log('Per-camera results:', cameraResults);
        console.log('Per-camera values:', cameraValues);

        setResponseMessage(cameraResults); // ✔ used for border colors
        setResponseValue(cameraValues); // ✔ numeric values / fallback status
        ...
        console.log('🎉 UI Updated:', {
          responseMessage: cameraResults,
          responseValue: cameraValues,
          rectanglesByCam
        });
        ...
        if (testing_result === 'OK') {
          ...
          setOkCount(updated_ok);
          setTCount(updated_total);
          console.log('✅ Test PASSED. OK count:', updated_ok);
        } else if (testing_result === 'NG') {
          const updated_ng = ng_count + 1;
          const updated_total = ok_count + updated_ng;
          setObjCount(nextCount);
          ...
          setNgCount(updated_ng);
          setTCount(updated_total);
          console.log('❌ Test FAILED. NG count:', updated_ng);
        }
      } else {
        // No object detected or incorrect object
        console.log('⚠️ Detection failed - stopping test');
        setShowStart(choosen_prof?.qrbar_check ? false : true);
        setQrbarShowStart(choosen_prof?.qrbar_check ? true : false);
        setShowstatus(false);
      }
  ...
  const getImageForCamera = (label, stageName) => {
    console.log('getstagenamebycamera', stageName);
    if (!groupedGallery || groupedGallery.length === 0) return null;

    // Filter images for this camera and stage
    const imagesForCamera = groupedGallery.filter(
      img => img.camera_label === label && img.stage_name === stageName
    );
                ...
                <div>
                  ...
                  <InputNumber
                    ...
                    onKeyDown={e => {
                      ...
                      const future = current + e.key;
                      const futureNum = Number(future);

                      if (futureNum > maxSampleCount) {
                        e.preventDefault();
                      }
                    }}
                  />
                    ...
                    <CardBody>
                      <h5 className=''>
                        ...
                        {qrbar_found === 0 ? (
                          ...
                          qrbar_found === 2 && (
                            <CheckCircleOutlined
                              className={`ms-3 ${qrbar_found === 2 && 'zoom-in'
                                }`}
                        ...
                        {qrbar_result === 0 ? (
                          <ExclamationCircleOutlined
                            className={`ms-3 ${qrbar_result === 0 && 'zoom-in'
                              }`}
                            style={{ color: 'deepskyblue', fontSize: '1.5rem' }}
                          />
                        ) : qrbar_result === 1 ? (
                          <CloseCircleOutlined
                            className={`ms-3 ${qrbar_result === 1 && 'zoom-in'
                              }`}
                            style={{ color: 'red', fontSize: '1.5rem' }}
                          />
                        ) : (
                          qrbar_result === 2 && (
                            <CheckCircleOutlined
                              className={`ms-3 ${qrbar_result === 2 && 'zoom-in'
                                }`}
                              style={{ color: 'green', fontSize: '1.5rem' }}
                            />
                          )
                        ...
                        {choosen_prof.detect_selection == true && (
                          <>
                            ...
                            {comp_found === 0 ? (
                              <ExclamationCircleOutlined
                                className={`ms-3 ${comp_found === 0 && 'zoom-in'
                                  }`}
                                style={{
                                  color: 'deepskyblue',
                                  fontSize: '1.5rem'
                              ...
                              <CloseCircleOutlined
                                className={`ms-3 ${comp_found === 1 && 'zoom-in'
                                  }`}
                                style={{ color: 'red', fontSize: '1.5rem' }}
                              />
                            ) : (
                              comp_found === 2 && (
                                <CheckCircleOutlined
                                  className={`ms-3 ${comp_found === 2 && 'zoom-in'
                                    }`}
                                  style={{ color: 'green', fontSize: '1.5rem' }}
                                />
                              )
                        ...
                        {comp_result === 0 ? (
                          ...
                          comp_result === 2 && (
                            <CheckCircleOutlined
                              className={`ms-3 ${comp_result === 2 && 'zoom-in'
                                }`}
                              style={{ color: 'green', fontSize: '1.5rem' }}
                            />
                          )
        ...
        <Modal isOpen={show_ratio} toggle={toggle} keyboard={false} centered>
          ...
          <ModalBody>
            ...
            <Table striped bordered hover className='mt-2'>
              ...
              <tbody>
                {ratio_data?.algo_ratio &&
                  Object.entries(ratio_data.algo_ratio).map(
                    ([stageKey, stageData]) => (
                      <tr key={stageKey}>
                        <td>
                          {stageData.stage_name} ({stageData.stage_code})
