...
const ManageStage = ({ history }) => {
  ...
  //         // ✔ Sort by comp_position
  //         const sorted = [...res.data[0].stages].sort(
  //           (a, b) => a.comp_position - b.comp_position
  //         );

  //         // ✔ Extract ordered stage IDs based on comp_position
  //         const stageIds = sorted.map(st => st._id);

  //         console.log("Preselected Stage IDs (ordered):", stageIds);
    ...
    const fetchData = async () => {
      try {
        ...
        const selected = stages
          .filter(s => s.comp_position !== null && s.comp_position !== undefined)
          .sort((a, b) => a.comp_position - b.comp_position)
          .map(s => s._id.$oid || s._id);
        ...
        // // ✔ Sort by comp_position ASC
        // const sortedStages = [...stages].sort(
        //   (a, b) => (a.comp_position ?? 999) - (b.comp_position ?? 999)
        // );

        // // ✔ Select stage IDs in this sorted order
        // const orderedStageIds = sortedStages.map(s => s._id.$oid || s._id);

        // console.log("Selected stages based on comp_position:", orderedStageIds);

        // setSelectedStages(orderedStageIds);
      } catch (error) {
        console.error("Error fetching train info:", error);
      }
    };
  ...
  const handleSelectAll = (e) => {
    const isChecked = e.target.checked;
    const currentPageItems = componentList.slice(
    ...
    setSelectedStages(prev => {
      ...
      if (isChecked) {
        updated = Array.from(new Set([...prev, ...currentPageIds]));
      } else {
        updated = prev.filter(id => !currentPageIds.includes(id));
      }

      updateComponentPositions(updated);
  ...
  const handleStageSelect = (stageId, isChecked) => {
    setSelectedStages(prev => {
      ...
      if (isChecked) {
        updated = [...prev, stageId];
      } else {
        updated = prev.filter(id => id !== stageId);
      }

      // Update componentList with comp_position
      updateComponentPositions(updated);
  ...
  const updateComponentPositions = (selectedIds) => {
    const newList = componentList.map(item => {
      const index = selectedIds.indexOf(item._id);

      return {
        ...item,
        comp_position: index !== -1 ? index + 1 : null
      };
    });
    ...
    switch (operation) {
      ...
      case 'TrainingMode':
        Swal.fire({
          ...
          text: `Are you sure you want to assign ${componentList.length} selected stages to Training Mode?`,
          icon: 'question',
          showCancelButton: true,
          confirmButtonColor: '#d33',
          cancelButtonColor: '#3085d6',
          ...
          if (result.isConfirmed) {
            sessionStorage.setItem('selectedStages', JSON.stringify(componentList));
            historyHook.push('/station_data_stg');
            console.log('Assigning stages to training mode:', componentList);
          }
        });
        break;
