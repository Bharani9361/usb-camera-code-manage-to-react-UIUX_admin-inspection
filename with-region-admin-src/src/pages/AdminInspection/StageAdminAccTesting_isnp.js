...
const StageAdminAccTesting = ({ history }) => {
    ...
    const [allIterationResults, setAllIterationResults] = useState([]);
    const [currentIterationIndex, setCurrentIterationIndex] = useState(0);
    const [totalIterations, setTotalIterations] = useState(0);
    const [isAllIterationsComplete, setIsAllIterationsComplete] = useState(false);

    const [iterationResultsIcons, setIterationResultsIcons] = useState({});
    ...
    const [allImages_count, setallImages_count] = useState(0);
    ...
    //         const response = await urlSocket.post("/api/stage/obj_detection_ver_stg_insp", formData, {
    //             headers: {
    ...
    //             await urlSocket.post('/api/stage/result_ver_stg_isnp',
    //                 {
    ...
    // const adminAccTestingd = useCallback(async (config) => {
    //     if (manual_abort || time_elapsed) return;
    ...
    //         // Find the max number of images across all cameras
    //         const maxImages = Math.max(
    //             ...modelData.map(cam =>
    //                 allImages.filter(img => img.camera_label.trim().toLowerCase() === cam.camera.label.trim().toLowerCase()).length
    //             )
    //         );
    ...
    //                 const imagesForCamera = allImages.filter(
    //                     img => img.camera_label.trim().toLowerCase() === labelName.trim().toLowerCase()
    //                 );

    //                 if (imagesForCamera[idx]) {
    //                     const imgObj = imagesForCamera[idx];
    //                     const imageSrc = imgObj.s3_path || imgObj.image_path || imgObj.localPath;
    ...
    //                         filename: imgObj.filename,  // use the actual dataset filename
    //                         imageSrc
    //                     });
    ...
    //             if (!capturedImages.length) continue;

    //             // Prepare FormData for this batch
    //             const formData = new FormData();
    //             formData.append('comp_name', modelData[0].comp_name);
    //             formData.append('comp_code', modelData[0].comp_code);
    //             formData.append('stage_name', modelData[0].stage_name);
    ...
    //             // Append captured images for this batch
    //             // capturedImages.forEach((img, i) => {
    //             //     formData.append(`datasets[${i}]`, img.imageSrc, img.filename);
    //             //     formData.append(`camera_labels[${i}]`, img.label);
    //             // });
    //             capturedImages.forEach((img, i) => {
    //                 formData.append(`datasets[${i}]`, img.imageSrc);
    //                 formData.append(`filenames[${i}]`, img.filename);
    //                 formData.append(`camera_labels[${i}]`, img.label);
    //             });
    ...
    //                 resultsByCamera[item.camera_label] = item.detection_result;
    //             });
    //             setRes_message(prev => ({ ...prev, ...resultsByCamera }));
    //             setShowstatus(true);
    ...
    //         Swal.fire({
    //             title: "Success!",
    //             text: "All camera testing completed.",
    //             icon: "success",
    ...
    // }, [manual_abort, time_elapsed, batch_id, config, initvalue1, modelData, slt_od_method, allImages, cont_apiCall]);
    ...
    //             ...modelData.map(cam =>
    //                 allImages.filter(
    //                     img => img.camera_label.trim().toLowerCase() === cam.camera.label.trim().toLowerCase()
    //                 ).length
    //             )
    //         );
    //         console.log('maxImages', maxImages, allImages)

    //         // ---- LOOP ALL IMAGE INDEXES ---- //
    //         for (let idx = 0; idx < maxImages; idx++) {

    //             const capturedImages = [];

    //             for (const cam of modelData) {
    //                 const labelName = cam.camera.label.trim().toLowerCase();

    //                 const imagesForCamera = allImages.filter(
    //                     img => img.camera_label.trim().toLowerCase() === labelName
    //                 );
    ...
    //                     capturedImages.push({
    //                         label: cam.camera.label,
    //                         version: cam.camera.model_ver,
    ...
    //             // ---- PREPARE FORM DATA ---- //
    //             const formData = new FormData();
    //             const md = modelData[0];

    //             formData.append("comp_name", md.comp_name);
    //             formData.append("comp_code", md.comp_code);
    //             formData.append("stage_name", md.stage_name);
    //             formData.append("stage_code", md.stage_code);
    //             formData.append("stage_id", md.stage_id);
    //             formData.append("model_name", md.model_name);
    //             formData.append("model_ver", md.model_ver);
    //             formData.append("model_id", md.model_id);
    //             formData.append("comp_id", md.comp_id);
    //             formData.append("comp_stg_model_id", md.comp_stg_model_id);

    //             formData.append("detect_type", slt_od_method || config.detection_type);
    //             formData.append("positive", config.positive);
    //             formData.append("negative", config.negative);
    //             formData.append("posble_match", config.posble_match);
    //             formData.append("upper_bound_conf", config.upper_bound_conf);
    ...
    //                 "/api/stage/obj_detection_ver_stg_insp",
    //                 formData,
    //                 { headers: { "content-type": "multipart/form-data" } }
    //             );
    ...
    //             setDetectionData(response.data);
    //             setIsOpen(true);
    //             console.log("capturedImagesData", capturedImagesData);
    ...
    //                 const md0 = modelData[0];
    //                 const resultPayload = {
    //                     comp_id: md0.comp_id,
    //                     comp_name: md0.comp_name,
    //                     comp_code: md0.comp_code,
    //                     stage_id: md0.stage_id,
    //                     stage_name: md0.stage_name,
    //                     stage_code: md0.stage_code,
    //                     model_name: md0.model_name,
    //                     model_ver: modelData.map(m => m.model_ver),
    //                     trainAccId: response.data._id,
    //                     logId: response.data.log_id,
    //                     thres: modelData.map(m => m.thres),
    //                     positive: config.positive,
    //                     negative: config.negative,
    //                     posble_match: config.posble_match,
    //                     ref_img_paths: capturedImages.map(img => img.imageSrc),
    //                     type: modelData.type,
    //                     position: modelData.position,
    //                     _id: modelData._id,
    //                     region_test_type,
    ...
    //                 detection.data.forEach(cameraObj => {
    //                     Object.entries(cameraObj).forEach(([label, data]) => {
    //                         if (data.OK > 0) camResults[label] = "OK";
    //                         else if (data.NG > 0) camResults[label] = "NG";
    //                         else camResults[label] = "PM";
    //                     });
    //                 });

    //                 setResponse_message(camResults);

    //                 const formattedData = detection.data.reduce((acc, item) => {
    //                     Object.entries(item).forEach(([key, val]) => {
    //                         acc[key.toLowerCase()] = val.results;
    //                     });
    //                     return acc;
    //                 }, {});
    ...
    //                 const initialSelections = {};
    //                 detection.data.forEach(cam => {
    //                     const label = Object.keys(cam)[0];
    //                     initialSelections[label] = camResults[label] || "OK";
    //                 });
    ...
    const findAccuracy = useCallback((cameraData) => {
        if (!Array.isArray(cameraData) || cameraData.length === 0) {
            return { both: 0, ok: 0, ng: 0 };
        }

        // ────────────────────────────────────────────────
        // REAL number of reviewed samples = agree + disagree
        // ────────────────────────────────────────────────
        const reviewedOK = _.sumBy(cameraData, d =>
            d.result === config[0].positive ? Number(d.agree || 0) + Number(d.disagree || 0) : 0
        );

        const reviewedNG = _.sumBy(cameraData, d =>
            d.result === config[0].negative ? Number(d.agree || 0) + Number(d.disagree || 0) : 0
        );

        const totalReviewed = reviewedOK + reviewedNG || 1;   // avoid division by zero
        ...
        // Overall accuracy = (all correct agreements) / (all reviewed samples)
        const train_acc_both = ((totalAgreeOK + totalAgreeNG) / totalReviewed) * 100;

        // Per-class accuracy
        const train_acc_ok = (reviewedOK > 0)
            ? (totalAgreeOK / reviewedOK) * 100
            : 0;

        const train_acc_ng = (reviewedNG > 0)
            ? (totalAgreeNG / reviewedNG) * 100
            : 0;

        // Optional: clamp to 100 (defensive)
        const clamp = v => Math.min(100, Math.max(0, v));

        return {
            both: clamp(train_acc_both),
            ok: clamp(train_acc_ok),
            ng: clamp(train_acc_ng)
        };
    }, [config]);


    // const adminAccTesting = useCallback(async (config, forcedIndex = null) => {
    //     if (manual_abort || time_elapsed) return;
    ...
    //             const label = img.camera_label.trim().toLowerCase();
    //             if (!imagesByCamera[label]) {
    //                 imagesByCamera[label] = [];
    //             }
    //             imagesByCamera[label].push(img);
    //         });

    //         const maxImages = Math.max(
    //             ...Object.values(imagesByCamera).map(imgs => imgs.length)
    //         );

    //         if (currentIterationIndex === 0) {
    //             setTotalIterations(maxImages);
    //             setAllIterationResults([]); // Reset on new test
    //             console.log(`Total iterations to process: ${maxImages}`);
    //         }

    //         const idx = forcedIndex !== null ? forcedIndex : currentIterationIndex;
    //         console.log(`\n========== PROCESSING ITERATION ${idx + 1}/${maxImages} ==========`);
    //         if (idx === 0) {
    //             setTotalIterations(maxImages);
    ...
    //         for (const cam of modelData) {
    //             const labelName = cam.camera.label.trim().toLowerCase();
    //             const imagesForCamera = imagesByCamera[labelName] || [];

    //             console.log(`🎥 Camera: ${cam.camera.label} (${labelName})`);
    //             console.log(`   Total images available: ${imagesForCamera.length}`);
    //             console.log(`   Requesting image at index: ${idx}`);

    //             if (imagesForCamera[idx]) {
    //                 const imgObj = imagesForCamera[idx];
    //                 const imageSrc = imgObj.s3_path || imgObj.image_path || imgObj.localPath;

    //                 console.log(` Found: ${imgObj.filename}`);
    //                 console.log(`   📍 Path: ${imageSrc}`);
    ...
    //                 });
    //             } else {
    //                 console.log(`   ⚠️ No image at index ${idx}`);
    //             }
    //         }
    //         if (capturedImages.length === 0) {
    //             console.log('No images for this iteration');
    //             return;
    //         }

    //         console.log(`Iteration ${idx + 1} - Captured:`, capturedImages);
    ...
    //             console.log("Result detection:", detection.data);
    //             setShowstatus(false);
    //             setShowresult(true);
    ...
    //             console.log('camResults', camResults);

    //             setResponse_message(camResults);
    //             setIterationResultsIcons(prev => ({
    //                 ...prev,
    //                 [currentIterationIndex]: camResults,   // store by iteration
    //             }));
    ...
    //             setAllIterationResults(prev => [
    //                 ...prev,
    //                 {
    //                     iterationIndex: idx,
    //                     collection: response.data._id, // Store collection ID
    //                     detectionData: detection.data,
    //                     results: formattedData
    //                 }
    //             ]);

    //             console.log(`⏸️ Iteration ${idx + 1} complete. Waiting for user submission...`);
    //         }

    //     } catch (error) {
    //         console.error('Error in adminAccTesting:', error);
    //         cont_apiCall();
    //     }
    ...
    //changes done for each result iterations 19 jan 2026
    const adminAccTesting = useCallback(async (config, forcedIndex = null) => {
        if (manual_abort || time_elapsed) return;
        ...
        try {
            ...
            if (hasNoObject || hasIncorrectObject) {
                ...
                setIterationResultsIcons(prev => ({
                    ...prev,
                    [idx]: detection.data.reduce((acc, camObj) => {
                        Object.entries(camObj).forEach(([label, data]) => {
                            acc[label.trim()] = data.compOnlyResult; // EXACT match to camera.label
                        });
                        return acc;
                    }, {})
                }));
    ...
    const ask_show = useCallback(async (data) => {
        console.log("hellosoldiers")
        setInitvalue2(prev => {
            const newCount = prev + 1;
            // setSample_count(newCount.toString());
    ...
    const agree_not = (data, datas, batch_id, result, sample_countfunc) => {
        ...
        try {
            urlSocket.post('/api/stage/agree_not_stg_insp', {
                '_id': data,
                'agree': datas,
                'batch_id': batch_id,
                'config': config,
    ...
    const handleSubmit = useCallback(() => {
        ...
        // ✅ STORE THIS ITERATION'S USER SELECTIONS
        const currentIterationData = allIterationResults[currentIterationIndex];

        setAllIterationResults(prev => {
            const updated = [...prev];
            updated[currentIterationIndex] = {
                ...updated[currentIterationIndex],
                userSelections: transformedData
            };
            return updated;
        });
        ...
        const nextIndex = currentIterationIndex + 1;
        console.log('nextIndex < totalIterations', nextIndex, totalIterations)
        setSample_count(nextIndex.toString())

        if (nextIndex < totalIterations) {
            // Move to next iteration
            console.log(`🚀 Moving to iteration ${nextIndex + 1}/${totalIterations}`);
            setCurrentIterationIndex(nextIndex);


            ask_show(transformedData);
            // processAllIterations()


            setTimeout(() => {
                adminAccTesting(config[0], nextIndex);
            }, 500);
            ...
            setIsAllIterationsComplete(true);

            // Call agree_not with combined data from all iterations
        }



    }, [userSelections, output_img_path, currentIterationIndex, totalIterations, allIterationResults, adminAccTesting, config, ask_show]);
    ...
    const processAllIterations = useCallback(() => {
        console.log('Processing all iterations:', allIterationResults);

        // ✅ COMBINE ALL USER SELECTIONS FROM ALL ITERATIONS
        const combinedUserSelections = {};

        allIterationResults.forEach((iteration, idx) => {
            if (iteration.userSelections) {
                Object.entries(iteration.userSelections).forEach(([camera, versions]) => {
                    if (!combinedUserSelections[camera]) {
                        combinedUserSelections[camera] = {};
                    }

                    // Merge versions from this iteration
                    Object.entries(versions).forEach(([version, selection]) => {
                        combinedUserSelections[camera][version] = selection;
                    });
                });
            }
        });

        console.log('Combined user selections from all iterations:', combinedUserSelections);

        // ✅ COMBINE ALL RESULTS FROM ALL ITERATIONS
        const combinedResults = {};

        allIterationResults.forEach((iteration) => {
            if (iteration.results) {
                Object.assign(combinedResults, iteration.results);
            }
        });

        // Get the last iteration's collection ID
        const lastIteration = allIterationResults[allIterationResults.length - 1];
        const collectionId = lastIteration?.collection;

        console.log('Calling agree_not with:', {
            collection: collectionId,
            data: combinedUserSelections,
            batch_id: batch_id,
            results: combinedResults,
            sample_count: totalIterations.toString()
        });

        // ✅ NOW CALL agree_not ONCE WITH ALL DATA
        agree_not(
            collectionId,
            combinedUserSelections,
            batch_id,
            combinedResults,
        ...
        setAllIterationResults([]);
        setIsAllIterationsComplete(false);

    }, [allIterationResults, batch_id, totalIterations, agree_not]);
    ...
    const renderResultSweetAlert = useCallback(({ type }) => {
        ...
        const transformedData = [];
        console.log('output_img_pathtransformedData', output_img_path)
                        ...
                        <Table bordered striped size="sm" style={{ fontSize: 12, marginBottom: 0 }}>
                            ...
                            <tbody>
                                {transformedData.map((row, index) => {
                                    ...
                                    const data = row.data;
                                    console.log('transformedData[index].captured_imaged)', row.version, index, row.data.captured_imaged)
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {
                                                console.log('transformedData.captured_imaged', transformedData.captured_imaged)
                                            }
    ...
    useEffect(() => {
        ...
        if (getModelData === undefined || getModelData === null) {
            ...
            if (parsedModelData?.allImages) {
                setAllImages(parsedModelData.allImages);
                // const cameraSet = new Set(parsedModelData.allImages.map(img => img.camera_label));
                // const cameraCount = cameraSet.size;

                // const totalImages = parsedModelData.allImages.length;

                // const perCamera = Math.floor(totalImages / cameraCount);

                // console.log("Camera Count:", cameraCount);
                // console.log("Total Images:", totalImages);
                // console.log("Images Per Camera:", perCamera);

                // console.log("cameraCount", cameraCount)

                // setallImages_count(perCamera);
            }
    ...
    useEffect(() => {
        console.log('config', config, sample_count)
        if (config.length > 0 && sample_count && Number(sample_count) === config[0].test_samples) {
    ...
    const showImage_insp = useCallback((output_img) => {
        console.log("Original Image URL:", output_img);
        const newUrl = image_url.replace("Vs_inspection/", "");
        const finalUrl = `${newUrl.replace(/\/$/, "")}/${output_img}`;
                                    ...
                                    <div style={{
                                        ...
                                        {uniqueCameras.map((camera) => {
                                            const iterationResult =
                                                iterationResultsIcons?.[currentIterationIndex]?.[camera.label];
                                            return (
                                                <div
                                                        ...
                                                        <div style={{
                                                            ...
                                                            <div style={{
                                                                ...
                                                                {cameraDisconnected && cameraDisconnected[camera.originalLabel] ? (
                                                                    ...
                                                                    <>
                                                                        {(() => {
                                                                            const cameraImages = allImages.filter(
                                                                                img => img.camera_label === camera.label
                                                                            );

                                                                            const idx = currentIterationIndex;

                                                                            if (idx >= cameraImages.length) return null;

                                                                            const match = cameraImages[idx];

                                                                            return (
                                                                                <img
                                                                                    src={showImage_insp(match.s3_path)}
                                                                                    style={{
                                                                                        position: "absolute",
                                                                                        top: 0,
                                                                                        left: 0,
                                                                        ...
                                                                        {show && msg && (
                                                                            ...
                                                                            {console.log('response_message4474', response_message)}
                                                                            {/* Result Icons */}
                                                                            {/* {iterationResultsIcons[currentIterationIndex] &&
                                                                            iterationResultsIcons[currentIterationIndex][camera.label]
                                                                            && (
                                                                                <div style={{
                                                                                    display: "flex",
                                                                            ...
                                                                            {iterationResult && (
                                                                                ...
                                                                                >
                                                                                    ...
                                                                                    {iterationResult === "PM" && (
                                                                                        <span style={{ color: "#fadb14", fontSize: "24px", fontWeight: "bold" }}>
                                                                                            PM
                                                                                        </span>
                                                                                    )}
                                                                                </div>
