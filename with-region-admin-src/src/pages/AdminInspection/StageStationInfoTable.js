...
function StageStationInfoTable(props) {
        ...
        const updatedSamples = samples.map(sample => {
            ...
            if (matched) {
                ...
                return {
                    ...
                    inspection_method: matched.inspection_method,
                };
            }
            return sample;
        });
    ...
    const openDetectionModal = (rowData, index) => {
        console.log("rowData", rowData, rectangles)
        setModalVisible(true);
        setCurrentDataIndex(index);
        setModalDetectionType(rowData.detection_type || 'ML');
        setModalSelectedRegions(
            rowData.selected_regions ||
            (Array.isArray(rectangles) ? rectangles.map(region => region.name) : [])
        );
...
export default StageStationInfoTable;
