...
const Stagelog = () => {
    const history = useHistory(); // No PropTypes needed!
    ...
    //     expandedRow: null,
    // });
    // console.log('state :>> 26', state);

    const [stage_code, setStageCode] = useState("");
    ...
    useEffect(() => {
        const stageData = JSON.parse(sessionStorage.getItem("stageData"));
    ...
    // const stageLog = async (stageData) => {
    //     let parent_comp_id  = stageData.parent_id;
    //     let stage_code = stageData.stg_code;
    //     console.log('stage_code :>> ', stage_code);
    //     let stage_name = stageData.stg_name;

    //     try {
    //         urlSocket.post('/stage_log_info', { 'parent_comp_id' : parent_comp_id,'stage_name': stage_name, 'stage_code': stage_code },
    //             { mode: 'no-cors' })
    //             .then((response) => {
    ...
    //                 setState(prevState => ({ ...prevState, log_data: data }));
    //             })
    //             .catch((error) => {
    //                 console.log(error);
    //             });
    ...
    const stageLog = async (stageData) => {
        try {
            const { stage_id, stage_code, stage_name } = stageData;
            console.log("Requesting stage log for:", stageData);

            const response = await urlSocket.post('/api/stage/stage_log_info', {

                stage_name,
                stage_code
            });

            console.log("Stage Log API response:", response.data);
    ...
    // const { log_data, expandedRow, currentPage, itemsPerPage } = state;
    const values = { log_data, expandedRow, currentPage, itemsPerPage };
                                                ...
                                                {currentItems.map((data, index) => (
                                                    <React.Fragment key={index}>
                                                        <tr onClick={() => toggleRow(index)}>
                                                            ...
                                                            <td style={{ backgroundColor: expandedRow === index ? "#d6d9f9" : "white" }}>
                                                                {data.date_time
                                                                    ? new Date(data.date_time?.$date || data.date_time).toLocaleString()
                                                                    : "N/A"}
                                                            </td>
...
// //     back = () => {
// //         this.props.history.push("/manageStages")
// //     }
...
// //   }).isRequired,
// //   // Add other props you're using as well
// // };
