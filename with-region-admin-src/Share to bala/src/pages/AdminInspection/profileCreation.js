...
const CompInfo = ({ history }) => {
  ...
  const allStageRectangles = React.useMemo(() => {
    if (!profile_data?.stg_info) return [];

    return profile_data.stg_info.flatMap(stage => {
      if (!stage.rectangles) return [];

      return Object.entries(stage.rectangles).flatMap(
        ([cameraName, regionsObj]) =>
          Object.values(regionsObj).map(region => ({
  ...
  console.log('allStageRectangles', allStageRectangles)
      ...
      <div className='page-content'>
                                              ...
                                              <div style={{ marginTop: 8 }} className='d-flex flex-column'>
                                                ...
                                                {data.detection_type === 'Smart Object Locator' && (
                                                  <Label style={{ color: '#555' }}>
                                                    Regions: {data.selected_regions?.length}
                                                  </Label>
                                                )}
        ...
        <Modal isOpen={modalVisible} toggle={() => setModalVisible(false)}>
          ...
          <ModalBody>
            ...
            {
              console.log('modalDetectionType', modalDetectionType, profile_data, modalSelectedRegions, showValidationMsg)
            }
              ...
              <div style={{ marginTop: '1rem' }}>
                ...
                >
                  ...
                  <div className='p-2 rounded' style={{ backgroundColor: 'white', border: '1px solid #ccc' }}>
                    ...
                    {allStageRectangles.map((region, idx) => {
                      const key = `${region.stage_id}:${region.camera}:${region.region_name}`;
                      ...
                      return (
                        <FormGroup check key={key}>
                          <Label check>
                            <Input
                              type="checkbox"
                              checked={modalSelectedRegions.includes(key)}
                              onChange={(e) => {
                                let updated = [...modalSelectedRegions];

                                if (e.target.checked) updated.push(key);
                                else updated = updated.filter(r => r !== key);
                            ...
                            <small style={{ marginLeft: 6, color: '#666' }}>
                              ({region.stage_name} / {region.camera})
                            </small>
                          </Label>
                        </FormGroup>
                      );
