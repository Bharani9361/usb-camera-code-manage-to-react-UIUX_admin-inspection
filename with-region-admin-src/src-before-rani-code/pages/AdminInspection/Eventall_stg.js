...
import {
    Checkbox, Radio, Space, Menu, TimePicker, Badge, Switch, InputNumber
} from 'antd';
...
function App(props) {
    ...
    console.log('data45 ', props)
    const data = props
    const { capture_mode, sub_mode } = props;
    console.log('capture_mode_sub_mode:', capture_mode, sub_mode);
    ...
    console.log('first52: ', sample_data, selected_data)
    ...
    const getSavedData = () => {
        ...
        if (savedData) {
            ...
            console.log('No saved data found, loading fresh data.', sample_data);
            setUsers(sample_data);
            loadSelected_comp(sample_data);
        }
    }
    ...
    const handleChange = (e) => {
        ...
        setUsers((prevUsers) => {
            const updated = prevUsers.map((user) => {
                if (user.comp_name + "_" + user.comp_code === name) {
                    // ✅ toggle checked value
                    return { ...user, checked };
                }

                // ✅ if unchecked, clean up dependent properties
                if (!checked && user.checked) {
                    const cleanUser = { ...user };
                    delete cleanUser.profile_name;
                    delete cleanUser.profile_id;
                    delete cleanUser.profile;
                    delete cleanUser.ok_allany;
                    delete cleanUser.ok_opt;
                    delete cleanUser.ng_allany;
                    delete cleanUser.ng_opt;
                    delete cleanUser.checked_name;
                    delete cleanUser.detect_selection;
                    delete cleanUser.qrbar_check;
                    delete cleanUser.qrbar_check_type;
                    cleanUser.qr_checked = false;
                    cleanUser.qruniq_checked = false;
                    cleanUser.radio_checked = false;
                    cleanUser.allow_user = false;
            ...
            const checkedCount = updated.filter((u) => u.checked).length;
            setSelectAll(checkedCount === updated.length);
            setPartialySelected(checkedCount > 0 && checkedCount < updated.length);
    ...
    const loadSelected_comp = (data) => {
        console.log('station_info', station_info, data)
        try {
            urlSocket.post('/station_comp_train_list', { 'station_id': station_info._id },
                { mode: 'no-cors' })
                .then((response) => {
    ...
    const checkCompSelected = (samples, select_data) => {
        ...
        // ✅ Convert comp_id objects to string IDs
        const selectedMap = new Map(
            select_data.map(item => [
                typeof item.comp_id === 'object' ? item.comp_id.$oid : item.comp_id,
                item
            ])
        );
        console.log('selectedMap', selectedMap);

        let checked_count = 0; // make sure this is initialized


        const updatedSamples = samples.map(sample => {
            // ✅ Convert _id to a string value too
            const sampleId = typeof sample.comp_id === 'object'
                ? sample.comp_id.$oid
                : sample.comp_id;

            const selected = selectedMap.get(sampleId);
            console.log('selected', selected);

            if (selected) {
                checked_count++;
                const updated = {
                    ...sample, checked: true, capture_mode: selected.capture_mode,
                    sub_mode: selected.sub_mode,
                    capture_count: selected.capture_count,
                    allow_user: selected.allow_user
                };
                console.log('updated_sample', updated);
                return updated;
            }
            return sample;
        });
    ...
    const selectManual_auto = (e, value, idx, str) => {
        ...
        if (str === '1') {
            if (users_data[idx].checked === true) {
                ...
                // ✅ Skip warning if mode is 'training'
    ...
    const submitForm = () => {
        const updatedUsers = users.map(user => ({
            ...
            allow_user: user.allow_user || false
    ...
    const createProfile = (isCreateProf) => {
        ...
        // sessionStorage.removeItem("selectedStationComp");f
        ...
        if (choosenProf !== null) {
            ...
            updated_comp.map((comp, comp_index) => {
                if (comp._id == profComp._id) {
                    ...
                    comp.profile = choosenProf.profile_data;
    ...
    const computeProfileRatio = async (prof_data) => {
        console.log('data955 ', choosenComp);
                    ...
                    <Table className="table mb-0 align-middle table-nowrap table-check" bordered>
                        <thead className="table-light">
                            <tr>
                                ...
                                {/* ✅ Dynamic heading: Training Mode / Manage Profile */}

                                {/* <th>Manage Profile</th> */}


                                {/* <th>Inspection Mode</th>
                <th>Object Detection Type</th> */}
                            </tr>
                        </thead>
                        ...
                        <tbody>
                            {users.map((user, index) => (
                                <tr key={index}>
                                    ...
                                    {/* <td className="bg-white">{user.profile_name || '- NA -'}</td> */}

                                    {/* ✅ Mode Column */}


                                    {/* {user.mode === 'inspection' && (
                    <td className="bg-white">
                      {data.comp_status !== false && (
                        <Button
                          className="btn btn-sm w-md"
                          color="info"
                          onClick={(e) => manageProfile(user, e, index, '1')}
                        >
                          Manage
                        </Button>
                      )}
                      ...
                      <Col>
                        {manualAuto.map((data, idx) => (
                          <Radio.Group
                            key={idx}
                            value={
                              user.radio_checked === true &&
                                user.checked_name === data.M
                                ? true
                                : false || undefined
                            }
                          >
                            <Space>
                              <div className="pay_cards">
                                <Radio
                                  onChange={(e) =>
                                    selectManual_auto(e, data, index, '1')
                                  }
                                  value={
                                    user.radio_checked === true &&
                                      user.checked_name === data.M
                        ...
                        <div style={{ marginTop: 8 }} className="d-flex flex-column">
                          ...
                          {user.detection_type === 'Smart Object Locator' && (
                            <Label style={{ color: '#555' }}>
                              Regions:{" "}
                              {user.selected_regions?.length > 0
                                ? user.selected_regions.join(', ')
                                : 'None'}
                            </Label>
                          )}
