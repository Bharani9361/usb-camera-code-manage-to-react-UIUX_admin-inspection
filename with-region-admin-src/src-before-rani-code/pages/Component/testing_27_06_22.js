...
class Inspection extends Component {
    ...
    componentDidMount = () => {
        ...
        positive = compData.config_data[0].positive
        negative = compData.config_data[0].negative
        posble_match = compData.config_data[0].posble_match
        ...
        this.setState({ _id: compData._id, config_data: config_data, capture_duration: Number(config_data[0].countdown) })
        this.setState({ testing_duration: test_duration, show_timer: true })
        try {
            axios.post('https://172.16.1.91:5000/batch_info', { 'comp_id': compData._id, 'comp_name': component_name1, 'comp_code': component_code1, 'batch_no': compData.batch_no, 'min_nof_test_sample': config_data[0].test_samples, 'inspect_type': config_data[0].inspection_type },
                { mode: 'no-cors' })
                .then((response) => {
    ...
    abortTesting = async (event) => {
        ...
        positive = comp_data.config_data[0].positive
        negative = comp_data.config_data[0].negative
        posble_match = comp_data.config_data[0].posble_match
        console.log('posble_match380', posble_match)
        try {
            axios.post('https://172.16.1.91:5000/abort', { 'batch_id': batch_id, 'comp_name': comp_name, 'comp_code': comp_code, 'positive': positive, 'negative': negative, 'posble_match':posble_match },
                { mode: 'no-cors' })
                .then((response) => {
    ...
    manual_abortTesting = async (event) => {
        ...
        try {
            axios.post('https://172.16.1.91:5000/manual_abort', { 'batch_id': batch_id, 'comp_name': comp_name, 'comp_code': comp_code, 'positive': positive, 'negative': negative, 'posble_match':posble_match},
                { mode: 'no-cors' })
                .then((response) => {
                    let aborted_data = response.data
                    console.log("aborted_data_326", aborted_data)
    ...
    agree_not = (data, datas, batch_id, result) => {
        ...
        try {
            axios.post('https://172.16.1.91:5000/agree_not', { '_id': data, 'agree': datas, 'batch_id': batch_id, 'comp_name': comp_name, 'comp_code': comp_code, 'positive': positive, 'negative': negative, 'result': result, 'posble_match':posble_match },
                { mode: 'no-cors' })
                .then((response) => {
                    let data = response.data
    ...
    navigateto = () => {
        ...
        let comp_code = component_code1
        try {
            axios.post('https://172.16.1.91:5000/status_change', { '_id': comp_id, 'comp_name': comp_name, 'comp_code': comp_code, 'status': 'admin approved trained model'},
                { mode: 'no-cors' })
                .then((response) => {
                    let data = response.data
    ...
    navigation = () => {
        ...
        let comp_code = component_code1
        try {
            axios.post('https://172.16.1.91:5000/status_update', { '_id': comp_id, 'comp_name': comp_name, 'comp_code': comp_code, 'status': 'training completed' },
                { mode: 'no-cors' })
                .then((response) => {
                    let data = response.data
                    console.log('approved', data)
        ...
        return (
            <React.Fragment>
                <div className="page-content">
                                                                    ...
                                                                    <div key={index}>
                                                                        {
                                                                            ...
                                                                            <CountdownTimer backgroundColor="#e6e46f" className="mt-2" size={28} hideDay={true} hideHours={false} border responsive count={this.state.testing_duration} onEnd={() => { this.onTimeup() }} />
                                                                        }
                                                                    </div>)):null
                                                            }
                                                        </Col>
                                                        ...
                                                        <div  style={{
                                                            ...
                                                            color:
                                                                this.state.response_message === "No Objects Detected" ? 'red' :
                                                                    this.state.response_message === positive? 'lightgreen' :
                                                                        this.state.response_message === negative ? 'red' :
                                                                            this.state.response_message === posble_match ? 'orange' :
                                                                                null
                                                        }}>
                                                            <br />
                                                ...
                                                }
                                                {
                                                    this.state.showresult ?
                                                        ...
                                                        <div className="containerImg" >
                                                            <div>
                                                                {
                                                                    this.state.response_message === positive &&
                                                            ...
                                                            {
                                                                this.state.response_message === negative &&
                                                                <div className="align-self-bottom">
                        ...
                        this.state.time_elapsed ?
                            ...
                            >
                                <div style={{ fontSize: '22px' }}>
                                    Testing process aborted as time elapsed
                                </div>
                            </SweetAlert> : null
                    }
                    {
                    ...
                    }
                    {
                        ...
                        >
                            <div style={{
                                fontSize: '20px',
                                color:
                                    this.state.response_message === positive ? 'green' :
                                        null
                            }}>
                    ...
                    {
                        this.state.response_message === posble_match &&
                        this.state.manual_abort !== true && this.state.time_elapsed !== true &&
                        // this.state.alertmsg ?
                        <SweetAlert
                            showCancel
