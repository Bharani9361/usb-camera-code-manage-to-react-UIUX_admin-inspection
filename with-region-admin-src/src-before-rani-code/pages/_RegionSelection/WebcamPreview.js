...
const WebcamPreview = () => {
    ...
    useEffect(() => {
        if (imageSrc || imageSrc && showRegion) {
            ...
            img.onload = () => {
                const drawFrame = () => {
                    ...
                    rectangles.forEach((rect, index) => {
                        ...
                        // Draw rectangle fill
                        // ctx.fillStyle = 'rgba(255, 0, 0, 0.3)'; // Example semi-transparent red fill
                        // ctx.fillRect(rect.x, rect.y, rect.width, rect.height);

                        const textPosX = index === movingRectangleIndex ? rect.x + 25 : rect.x + 10;
                        // Draw text with thicker black stroke
                        ...
                        // Draw text with thinner white stroke
                        ctx.lineWidth = 3;
                        ctx.strokeStyle = 'black';
                        ctx.strokeText(rect.name, textPosX, rect.y + 15);
                        ctx.fillStyle = 'white';
                        ...
                        const trashButton = trashButtonsRef.current[index];
                        if (trashButton) {
                            trashButton.style.left = `${rect.x + rect.width - 24}px`;
                            trashButton.style.top = `${rect.y}px`;
                        }
                    });
                };
    ...
    const handleMouseDown = (e) => {
        ...
        setSelecting(true);
        // rectangleRef.current = { x, y, width: 0, height: 0, name: rectangles.length+1, id: uuidv4(), nestedRectangles: [] }
        rectangleRef.current = { x, y, width: 0, height: 0, name: rectNameInput, id: uuidv4(), colour: selectedColor }
    };
    ...
    const handleMouseUp = () => {
        if (selecting) {
            if (nestedRectangleRef.current) {
                ...
                const { x, y, width, height, name, id, colour } = rectangleRef.current;
                if (width < 30 && height < 30) {
                    console.log('Rectangle is too small');
                    ...
                    setRectangles(prevRectangles => {
                        const updatedRectangles = [...prevRectangles];
                        let updatedname = updatedRectangles.length + 1
                        // let updatedname = (!name) ? `${updatedRectangles.length + 1}` : name
                        const rectNameFound = prevRectangles.some((item, item_id) => item.name === updatedname)

                        updatedRectangles.push({ x, y, width, height, name: rectNameFound ? updatedname+1 : updatedname, id, colour });
                        return updatedRectangles;
                    });
                    rectangleRef.current = null;
                    setRectNameInput('');
    ...
    const handleNameSubmit = (e, index) => {
        const isNameFound = rectangles.some((item, index) => item.name === inputRectangleName && index !== editingRectangleIndex);
        if(isNameFound) {
            // e.preventDefault();
            console.log('Rectangle Name already exist');
                ...
                <div className='d-flex'>
                    <div style={{ position: 'relative' }}>
                        <canvas
                            ...
                            width={640}
                            height={480}
                            onMouseDown={showRegion ? handleMouseDown : null}
                            onMouseMove={showRegion ? handleMouseMove : null}
                            onMouseUp={showRegion ? handleMouseUp : null}
                        >
                        </canvas>
                        {
                    ...
                    /> */}
                        ...
                        <InputGroup>
                            ...
                            <ColorPicker_1 selectedColor={selectedColor} setSelectedColor={setSelectedColor} />
                        </InputGroup>
                        <InputGroup className='mx-3 my-3'>
                            <Label check className='py-1 px-2' style={{userSelect: 'none', boxShadow: showRegion ? '0px 0px 10px 5px green' :'0px 0px 10px 5px #ccc', borderRadius: '5px'}}>
                                <Input
                                    type="checkbox"
                                    checked={showRegion}
                                ...
                                (
                                    <div className='ms-3 d-flex flex-column'>
                                        <InputGroup>
                                            ...
                                            />
                                            <Button className='' color='success' onMouseDown={() => handleNameSubmit(id_rect)}>
